...
export default function Config() {
  ...
  const [sosRecivers, setSOSRecivers] = useState(
    [] as { name: string; phone: string }[]
  );
  const [reciverRegisterModalVisibility, setReciverRegisterModalVisibility] =
    useState(false);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
      ...
      <View style={configStyles.page}>
        <ScrollView
          contentContainerStyle={configStyles.scrollableContent}
          style={configStyles.scrollableContainer}
        >
          <Text style={configStyles.title}>세이프 워드 설정</Text>
          <Text style={configStyles.description}>
            AI와 전화 도중 세이프 워드를 언급하면
          </Text>
          ...
          <View style={configStyles.form}>
            ...
            <View>
              <Text style={configStyles.safeWordCountTitle}>
                세이프 워드 반복 횟수
              </Text>
              <View style={configStyles.optionList}>
                <Select
                  selection={safeWordTriggerCount}
                  onSelect={(option) =>
                    typeof option === "number" &&
                    setSafeWordTriggerCount(option)
                  }
                >
                  <Option option={2}>
                    <Text>2회</Text>
            ...
            <View style={configStyles.switchsBox}>
              <View>
                <View style={configStyles.switchBoxHeader}>
                  <Text style={configStyles.switchBoxTitle}>
                    SOS 메시지 보내기
                  </Text>
                  <Switch
                    isOn={shouldSendSOS}
                    onToggle={setShouldSendSOS}
                    style={configStyles.switch}
                  />
                </View>
                <Text style={configStyles.switchBoxContent}>
                  세이프 워드를 반복했을 때,
                </Text>
                <Text style={configStyles.switchBoxContent}>
                  내가 지정해둔 사람들에게 SOS 메시지를 신속하게
                </Text>
                <Text style={configStyles.switchBoxContent}>
                  보낼 수 있습니다.
                </Text>
              ...
              <View>
                <View style={configStyles.switchBoxHeader}>
                  ...
                  <Switch
                    isOn={shouldAttachAudio}
                    onToggle={setShouldAttachAudio}
                    style={configStyles.switch}
                  />
                </View>
                <Text style={configStyles.switchBoxContent}>
            ...
            <View style={configStyles.sosReciverBox}>
              <View style={configStyles.sosReciverModalOpener}>
                <Text style={configStyles.sosReciverModalTitle}>
                  SOS 메시지 받는 사람
                </Text>
                <Pressable
                  onPress={() => setReciverRegisterModalVisibility(true)}
                >
        ...
        <Button
          onPress={() => router.navigate("/safe-call/select")}
          style={configStyles.next}
        >
          <Text style={configStyles.buttonText}>다음</Text>
        </Button>

        <ReciverAddModal
          visible={reciverRegisterModalVisibility}
          onRequestClose={(reciver) => {
...
function ReciverAddModal({
  visible,
  onRequestClose,
  style,
}: ReciverAddModalProps) {
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");

  const isValid =
    name.length > 0 && /^(?:\d{3}-\d{4}-\d{4}|\d{11})$/.test(phone);
    ...
    >
      <View style={reciverAddModalStyles.container}>
        <View style={reciverAddModalStyles.modal}>
          ...
          <Text style={reciverAddModalStyles.title}>
            SOS 메시지 받는 사람 추가
          </Text>
          <TextInput
            value={name}
            ...
            cursorColor="#6028FF"
            style={reciverAddModalStyles.textInput}
          />
          <TextInput
            value={phone}
          ...
          >
            <Text
              style={[
                reciverAddModalStyles.saveText,
